...
func _physics_process(delta: float) -> void:
	if ShakeStrength > 0.0:
		sprite.position = spriteog + Vector2(randf_range(-ShakeStrength,ShakeStrength),randf_range(-ShakeStrength,ShakeStrength))
	...
	if Input.is_action_just_pressed("interact") and player_near and not alrdy_use:
		...
		PlayerItemDisplay.ItemSpawn("Washing")
...
func _on_body_entered(body: Node2D) -> void:
	if body.is_in_group("player") and not col_hitbox.disabled and not alrdy_use:
		act_label.show()
